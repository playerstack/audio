...
describe('createReactElement', () => {
  ...
  test('reflects a declared attribute prop to the element attribute after effects (Req 7.2)', () => {
    const PlayButton = createReactElement(BINDING);
    const { el } = renderElement(PlayButton, { 'aria-label': 'Play audio' });

    expect(el.getAttribute('aria-label')).toBe('Play audio');
  });
  ...
  test('does not re-run syncProperty for a deep-equal object prop across renders (Req 16.1, 7.5)', () => {
    ...
    // Deep-equal rerender on the SAME instance: no additional config sync because
    // useDeepCompareMemoize keeps the same stable reference (Req 16.1, 7.5, R1).
    ...
    // A real content change re-runs the property-sync effect.
