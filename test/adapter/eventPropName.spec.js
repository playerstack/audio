...
describe('eventPropName', () => {
  describe('eventNameToPropName', () => {
    test('maps a prefixed request event to an on* callback prop name (Req 16.x)', () => {
    ...
    test('drops the playerstack- prefix and PascalCases the remaining segments', () => {
      expect(eventNameToPropName('playerstack-seek-request')).toBe('onSeekRequest');
      expect(eventNameToPropName('playerstack-ad-skip')).toBe('onAdSkip');
      expect(eventNameToPropName('playerstack-prev-request')).toBe('onPrevRequest');
    });
    ...
    test('ignores empty segments produced by trailing/leading/double hyphens', () => {
      expect(eventNameToPropName('playerstack-next-request')).toBe('onNextRequest');
